...
function CurrentStateAnalysis() {
  ...
  const [formData, setFormData] = useState<IndustryAnalysisForm>({
    industry: '',
    targetMarket: '',
    operationType: '',
    scope: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://omar44.app.n8n.cloud/webhook/current-state-of-industry', {
        method: 'POST',
        headers: {
      ...
      <div className="max-w-3xl mx-auto">
        <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-semibold">Current State of Industry Analysis</h2>
            <button 
            ...
            <div className="mt-8 space-y-6 bg-white p-6 rounded-lg shadow-sm border border-gray-100">
              ...
              {aiResponse.currentState && (
                <div className="space-y-2">
                  ...
                  <p className="text-gray-600">{aiResponse.currentState}</p>
                </div>
              )}

              {aiResponse.trends && aiResponse.trends.length > 0 && (
                <div className="space-y-2">
                  <h4 className="font-medium text-gray-700">Current Trends</h4>
                  <ul className="list-disc list-inside space-y-1">
                    {aiResponse.trends.map((trend, index) => (
              ...
              {aiResponse.opportunities && aiResponse.opportunities.length > 0 && (
                <div className="space-y-2">
                  <h4 className="font-medium text-gray-700">Market Opportunities</h4>
                  <ul className="list-disc list-inside space-y-1">
                    {aiResponse.opportunities.map((opportunity, index) => (
              ...
              {aiResponse.challenges && aiResponse.challenges.length > 0 && (
                <div className="space-y-2">
                  <h4 className="font-medium text-gray-700">Industry Challenges</h4>
                  <ul className="list-disc list-inside space-y-1">
                    {aiResponse.challenges.map((challenge, index) => (
